...
void CpService::checkPerceivedObjects(const SimTime& T_now)
{
    ...
    */
    if (mObjectInclusionConfig == 0) {
        ...
        for(const auto& objEntry : mLocalDynamicMap->allObjectEntries()) {
            ...
            mSelectedObjects.push_back(std::move(objPtr));
        }
    ...
    */
    ...
    std::vector<std::shared_ptr<const ObjectData>> typeA;
    ...
    for (const auto& objEntry : mLocalDynamicMap->allObjectEntries()) {
        ...
        // int perceptionQuality = objectPerceptionQuality(objPtr, T_now); // TODO: change the way this value is obtained (this should be updated every cycle and only here is only accessed)
        // if (perceptionQuality < mObjectPerceptionQualityThreshold) {
        //     continue;
        // }

        const SimTime& firstDetection = objPtr->firstDetection;
        const SimTime& lastDetection = objPtr->lastDetection;
        ...
        const vanetza::geonet::StationType& stationType = objPtr->stationType;
        ObjectClass_t objClass = StationType2ObjectClass(stationType);
        ...
        if(isTypeA) {
            typeA.push_back(std::move(objPtr));
    ...
    // std::sort(mSelectedObjects.begin(), mSelectedObjects.end(), [this](std::shared_ptr<const ObjectData> a, std::shared_ptr<const ObjectData> b) {
...
void CpService::sendCpm(const SimTime& T_now)
{
    ...
    std::vector<const vanetza::asn1::Cpm> messages;
    ...
    if(mMessageAssemblyConfig == 0) {
        ...
        if(mSelectedObjects.size() > 0) {
            ...
            // std::sort(mSelectedObjects.begin(), mSelectedObjects.end(), [this,T_now](std::shared_ptr<const ObjectData> a, std::shared_ptr<const ObjectData> b) {
        ...
        while(mOffloadedObjects.size() > 0) {
            ...
            // Re-select perceived objects for transmission in the currently generated CPM
            mSelectedObjects.clear();
            auto start = mOffloadedObjects.begin();
            auto end = mOffloadedObjects.size() > mMaxPerceivedObjects ?
                        mOffloadedObjects.begin() + mMaxPerceivedObjects :
                        mOffloadedObjects.end();
            mSelectedObjects.insert(mSelectedObjects.end(), start, end);
            mOffloadedObjects.erase(start, end);

            // while(mSelectedObjects.size() < mMaxPerceivedObjects && mOffloadedObjects.size() > 0) {
            //     mSelectedObjects.push_back(mOffloadedObjects.front());
            //     mOffloadedObjects.erase(mOffloadedObjects.begin());
            // }
...
void addPerceivedObjectContainer(vanetza::asn1::Cpm& message, const VehicleDataProvider& vdp, const CpService::ObjectList& selectedObjects, const int& numberOfPerceivedObjects)
{
    ...
    for (const auto& objPtr : selectedObjects) {
        ...
        *(po->objectId) = 1; // TODO: implement object id map system
        ...
        SimTime lastDetection = objPtr->lastDetection;
        int64_t deltaTime = lastDetection.inUnit(SIMTIME_MS) - referenceTime;
